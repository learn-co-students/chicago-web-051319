# Thinking In React (And Pets Review)

What could it mean to "think" in HTTP?

What could it mean to "think" in REST?

What could it mean to "think" in OOP?

What could it mean to "think" in React?
...
1) Break the UI into
  its individual components
    as well as hierarchy between those components

2) Build a static version in react
  no state work yet, just props
  top down (parent components first) if project is small
  bottom up (child components first) for more structured testing / larger projects

3) Identify minimal but complete representation of UI state
  state = info that will change with user input
  minimal = not creating redundant state, like a "count" attribute when you have an array already

4) Decide who should own state
  For each piece of state in your app
    identify all components that need to use that state
    find the first common parent of those components; the state needs to live at least that high up

5) Add inverse data flow
  parent holding state defines function responsible for altering that state
  parent passes data down to child along with the function
  child calls the function during the appropriate event...
  which updates the parent state... <---INVERSE FLOW! child flowing to parent
